...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            ...
            -- Will be called by the trigger to compute the changes for the event detail
            ...
            BEGIN
                ...
                AND sys_period < building.sys_period -- Not strictly required here, but if we want to be able to call this on history rows we will need it
            ...
            BEGIN
                IF building.event_id IS NULL THEN
                    RAISE EXCEPTION 'Missing required event_id parameter';
                END IF;

                IF building.event_type IS NULL THEN
                    RAISE EXCEPTION 'Missing required event_type parameter';
                END IF;

                IF building.event_origin IS NULL THEN
                    RAISE EXCEPTION 'Missing required event_origin parameter';
                END IF;

                IF building.point IS NULL THEN
                    RAISE EXCEPTION 'Missing required point parameter';
                END IF;
            ...
            -- ⚠️ Naming is paramount here: it must be named like this to make sure it executes after the versioning trigger
            CREATE TRIGGER building_versioning_trigger__1_insert_or_update_event_detail_trigger
            AFTER INSERT OR UPDATE ON batid_building
            FOR EACH ROW EXECUTE FUNCTION upsert_event_detail_trigger();
            """,
            reverse_sql="""
            DROP TRIGGER IF EXISTS building_versioning_trigger__1_insert_or_update_event_detail_trigger ON batid_building;
