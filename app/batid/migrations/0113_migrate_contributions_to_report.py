...
start_summer_game = datetime(2024, 7, 3, tzinfo=timezone.utc)
end_summer_game = datetime(2024, 9, 3, tzinfo=timezone.utc)


def validate_contribution(contribution: Any) -> None:
...
def is_rejected_empty_contribution(contribution: Any) -> bool:
    return (
...
def migrate_single_contribution(contribution: Any) -> None:
    ...
    report.save()
    if (
        new_report_created_at >= start_summer_game
        and new_report_created_at <= end_summer_game
    ):
        report.tags.set(["Signalement utilisateur", "Jeu de l'été 2024"])
    else:
        report.tags.set(["Signalement utilisateur"])
    messages = []
