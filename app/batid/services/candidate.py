...
class Inspector:
    ...
    def get_candidate(self):
        q = f"SELECT sub.* FROM (SELECT id, ST_AsEWKB(shape) as shape, source, source_version, source_id, address_keys, is_light, inspected_at, random  FROM {Candidate._meta.db_table} WHERE inspected_at IS NULL LIMIT 10000) as sub ORDER BY sub.random asc LIMIT 1 FOR UPDATE SKIP LOCKED"
