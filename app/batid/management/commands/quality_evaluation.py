...
class Command(BaseCommand):
        ...
        for element in data.get('elements', []):
            if 'tags' in element:
                building = {
                    ...
                    'street_rep': self._extract_street_rep(element['tags'].get('addr:housenumber', '')),
    ...
    def _display_results(self, stats: Dict):
        ...
        self.stdout.write(f"Addresses missing in our database: {stats['missing_in_our_db']}")
    ...
    def normalize_street(self, street):
        if not street:
            return ''
        # Remove diacritics
        normalized = ''.join(c for c in unicodedata.normalize('NFKD', street)
