...
from batid.services.geocoders import GeocodeEarthGeocoder, NominatimGeocoder, PhotonGeocoder, BanGeocoder
...

{
    ...
    def report(self, tests):
        ...
        parsed_data = {f"{item['name']} // {item['address']}": {geocoder: result['valid'] for geocoder, result in item['geocoding'].items()} for item in tests}
        ...
        parsed_data = {f"{item['name']} // {item['address']}": {geocoder: result['distance'] for geocoder, result in item['geocoding'].items()} for item in tests}
        df = pd.DataFrame(parsed_data).T
